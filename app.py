...
def serve_points():
    ...
    if request.method == 'POST':
        ...
        for v in [ '_mod', '_hi', '_xhi' ]:
            kw_pts['pct' + v] = float(ranged(request.form, 'v' + v, 'cutoff')) \
                                / 100.0
        ...
        if request.form.get('expr'):
            ...
            for e_dot in [d for d in re.split(r'\.+', e) if d]:
                try:
                    e_commas_u = [int(c) for c in re.split(',+', e_dot) if c]
                except ValueError:
                    continue
                ...
                ncals = sum(map(lambda e: int(hpv_vals[e - 1].cals), e_commas))
...
def serve_zipsplit():
    ...
    if request.method == 'POST':
        ...
        # Process files
        files = request.files.getlist("files[]")
        zip_name, zip_data = zip_all_splits((f.stream for f in files), hr_min = hr_min)
